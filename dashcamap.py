...
parser.add_argument('--skip-cleanup', 	action='store_true', 	help='Skip cleanup step and use existing generated frames.')
parser.add_argument('--no-markers', 	action='store_true', 	help="Don't include markers in the generated map.")
...
if args.flush:
	...
	os.remove('recognized2.txt')
	os.remove('recognized3.txt')
	os.remove('map.html')
	print("Deleting frames and coords.")
	quit()

# --- VARIABLES ---
regexCoords = re.compile(r"\b\d{1,3}\sKM/H\sN\d{2}\.\s*\d{6}\sE\d{2}\.\s*\d{6}\b")
coords = []
lat = 0.0
lon = 0.0
frameCount = 0
...
# ----- FRAME EXTRACTION -----
# Extract frames that include GPS info, usually updated every second.
...
if not args.skip_process:
	...
	while frameCount < totalFrameCount:
		...
		if len(sys.argv) > 2:
			if sys.argv[2] == '-v' or '--verbose':
				print(f"Processing frame {frameCount}")

		# Apply grayscale filter, blur, thresholding, morph open, and inversion
		gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
		#blur1 = cv2.medianBlur(gray,3)
		#blur2 = cv2.medianBlur(gray,91)
		#divided = np.ma.divide(blur1, blur2).data
		#norm = np.uint8(255*divided/divided.max())
		#th, threshed = cv2.threshold(norm, 100, 255, cv2.THRESH_OTSU)
		th, threshed = cv2.threshold(gray, 100, 255, cv2.THRESH_OTSU)

		#dst = np.vstack((gray, blur1, blur2, norm, threshed)) 
...
with open("recognized2.txt", "a+") as file2:
	for line in lines:
		...
		if match:
			regexN = re.search(r"N(\d+\.\s*\d+)", line)
			regexE = re.search(r"E(\d+\.\s*\d+)", line)
			
			if regexN:
				lat = float(regexN.group(1).replace(" ", ""))

			if regexE:
				lon = float(regexE.group(1).replace(" ", ""))

			if regexN and regexE:
